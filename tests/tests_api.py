...
from settings import valid_page, valid_per_page
from settings import valid_id, invalid_id
from settings import valid_reg_body, valid_login_body, valid_put_body
...
# def test_put_api_users_id(body=valid_put_body):
#     status, result = tr.put_api_users_id(body)
#     assert status == 200
#     assert "token" and "id" in result